 ...
 * Use --product to change product name, default is collaboration-engine-internal
 */

const fs = require('fs');
const https = require("https");
const exec = require('util').promisify(require('child_process').exec);
let product = 'Collabration Engine'
let from, to, version, ceVersion, pomVersion, currentVersion;
...
async function getReleases() {
  ...
  const ceVersion = pomVersion.replace(/(\.\d+|-SNAPSHOT)$/, '');
...
function createLink(type, id, char) {
  return id ? `[${char ? char : id}](${createGHLink(`collaboration-engine-internal/${type}/${id})`)}` : '';
}
...
### No changes since [${from}](https://github.com/vaadin/collaboration-engine-internal/releases/tag/${from})
  ...
  console.log(`\n  - [⧉ All changes](https://github.com/vaadin/collaboration-engine-internal/compare/${from}...${currentVersion})`)
}
