...
If you have a Spring application, you should register the `VaadinServiceInitListener` by marking it as a `@Component`.
...
The code for non-Spring applications is similar, but instead of registering the listener with `@Component`, you need to register it via Java SPI loading facility.
...
```
java -Dvaadin.ce.dataDir=/Users/peppe/.vaadin/collaboration-engine -jar ce-dx-test-1.0-SNAPSHOT.jar
```
