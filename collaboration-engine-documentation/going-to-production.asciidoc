...
In the following example we have a Spring project, so the handler is configured in a singleton-scoped
Spring component implementing the `VaadinServiceInitListener` interface.
...
```java
@SpringComponent
public class CollaborationEngineConfiguration implements VaadinServiceInitListener {
    ...
    public void serviceInit(ServiceInitEvent serviceEvent) {
        ...
        CollaborationEngine.getInstance(service).setLicenseEventHandler(licenseEvent -> {
