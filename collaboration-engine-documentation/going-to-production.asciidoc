...
the class as a `@SpringComponent`. In the `serviceInit` method, you can then create a `CollaborationEngineConfiguration` object and set
the data directory path with the `setDataDir` method.
