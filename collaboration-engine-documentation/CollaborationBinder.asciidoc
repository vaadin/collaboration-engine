...
When the field's value type is a collection, which usually mean the field is some
sort of a multi select, you must use the `forCollectionField` method. It requires
both the type of the collection, as well as the type of the elements inside it.

For example, the value type of `CheckboxGroup` is `Set`.
In the example below, we must provide the collection type `Set` and the element type `String`.
[source, java]
----
